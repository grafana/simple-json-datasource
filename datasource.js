...
function (angular, _, dateMath, kbn) {
  ...
  module.factory('GenericDatasource', function($q, backendSrv, templateSrv) {
    ...
    GenericDatasource.prototype.performTimeSeriesQuery = function(queries, start, end) {
      var reqBody = {
        metrics: queries,
        cache_time: 0
      };
     ...
     * @returns {*|Promise}
     */
    GenericDatasource.prototype._performMetricSuggestQuery = function(metric) {
      var options = {
    ...
    GenericDatasource.prototype._performMetricKeyLookup = function(metric) {
    ...
    GenericDatasource.prototype._performMetricKeyValueLookup = function(metric, key) {
      if(!metric || !key) {
        return $q.when([]);
      }
    ...
    GenericDatasource.prototype.performTagSuggestQuery = function(metric) {
      var options = {
        url: this.url + '/api/v1/datapoints/query/tags',
        method: 'POST',
        data: {
    ...
    function convertTargetToQuery(options, target) {
      ...
      if (target.downsampling !== '(NONE)') {
        ...
        query.aggregators.push({
          ...
          sampling: GenericDatasource.prototype.convertToKairosInterval(target.sampling || options.interval)
        });
      }
        ...
        if (target.nonTagGroupBys) {
          _.each(target.nonTagGroupBys, function(rawGroupBy) {
            ...
            if (formattedGroupBy.name === 'time') {
              formattedGroupBy.range_size = GenericDatasource.prototype.convertToKairosInterval(formattedGroupBy.range_size);
            }
    ...
    GenericDatasource.prototype.convertToKairosInterval = function(intervalString) {
